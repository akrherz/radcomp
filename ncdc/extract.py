...
def main():
    """Go Main Go."""
    ts = datetime(int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]))

    os.chdir("/tmp/")
    # Extract tar file
    cmd = (
        "tar -xf /mnt/mtarchive/data/nexrad3_archive/%s/N3_N0R_NET_%s.tar"
        % (
            ts.year,
            ts.strftime("%Y%m%d"),
        )
    ...
    for fn in files:
        p = subprocess.Popen(
            "unzip -o " + fn, shell=True, stdout=subprocess.PIPE
        )
        lines = p.stdout.readlines()
        for line in lines:
            if (
                line.find("extracting:") == -1
                and line.find("inflating:") == -1
            ):
                continue
            l3 = line.split()[1]
