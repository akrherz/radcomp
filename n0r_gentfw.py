...
sql2 = "INSERT into nexrad_n0r_tindex( the_geom, datetime, filepath) values \
  (GeomFromEWKT('SRID=4326;MULTIPOLYGON(((-126 50,-66 50,-66 24,-126 24,-126 50)))'), '%s', '/mesonet/ARCHIVE/data/%s/GIS/uscomp/n0r_%s.png')" % (ts.strftime("%Y-%m-%d %H:%M"), ts.strftime("%Y/%m/%d"), ts.strftime("%Y%m%d%H%M") )
